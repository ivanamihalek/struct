...
static int **intmatrix(int rows, int columns){
  if ((rows == 0) || (columns == 0)) {
    fprintf (stderr,"number of rows/cols 0 in intmatrix().\n");
    return NULL;
  }
  void *ptr = calloc(rows * columns, sizeof(int));
  if (ptr == NULL)  {
    fprintf (stderr, "allocation failed for intmatrix().\n");
    return NULL;
  }
  return ptr;
}

static void free_imatrix(int **m) {
    free(m);
}


static int string_clean ( char* string, int length) {
    int ctr;
...
static char single_letter (char code[]); /* defined at end of file */
...
int readPDB(const char *pdbname,  char chain, Protein * protein) {
    ...
    Residue * sequence;
    Helix   * helix, *helix_new;
    Strand  * strand, *strand_new;
    FILE    * fptr = NULL;
    int     * sse_sequence;
    int     ** sse_sub_elements;
    char line[BUFFLEN];
    ...
    /* Maybe this could go elsewhere but */
    /* here seems guaranteed to happen.  */
    for (resctr = 0; resctr < no_res; resctr++)
      sequence[resctr].helix_type = HELIX_TYPE_UNDEF;
	...
	if( ! strncmp(line,"ATOM", 4) ||  ! strncmp(line,"HETATM", 6)){
	    ...
	    memset ( tmp, 0, PDB_ATOM_ATOM_NAME_LEN+8);
	    /* skip initial blanks*/
	    ctr  = 0;
    ...
    for (resctr=0; resctr < no_res; resctr ++ ) {
	retval = string_clean (sequence[resctr].pdb_id, PDB_ATOM_RES_NO_LEN+2);
	if ( retval ) {
    ...
    /*****************************/
    /*                           */
    /*  HELIX/STRAND Annotation  */
    /*                           */
    /*****************************/

    /*
    for (resctr=0; resctr < no_res; resctr ++ )
      fprintf(stderr, "'%s' (%s)\n", sequence[resctr].pdb_id, sequence[resctr].res_type);
    */

    /*********************************************/
    /*  Try Stride first.                        */
    /*  Assume 'old_chain' still reflects chain  */
    /*  that was read in from the ATOM records.  */
    /*********************************************/
    rewind(fptr);
    errmsg = calcStride(sequence, no_res, fptr, pdbname, old_chain);

    /************************************/
    /*  Otherwise, try PDB file itself  */
    /*************************************/
    if (errmsg) {
      fprintf(stderr, "WARNING: ");
      pdbnamelen = strlen(pdbname);
      if (pdbnamelen >= 16)
	fprintf(stderr, "...%s: ", &(pdbname[pdbnamelen - 16])); 
      else
	fprintf(stderr, "%s: ", pdbname); 
      fprintf(stderr, "%s: trying PDB header instead\n", errmsg);
	...
	if( ! strncmp(line,"ATOM", 4) ||  ! strncmp(line,"HETATM", 6)){
	  ...
	  if (strncmp(" 1", line + PDB_HELIX_CLASS, PDB_HELIX_CLASS_LEN) == 0)
	    helix_type = HELIX_TYPE_ALPHA;
	  else if (strncmp(" 5", line + PDB_HELIX_CLASS, PDB_HELIX_CLASS_LEN) == 0)
	    helix_type = HELIX_TYPE_310;
	  else if (strncmp(" 3", line + PDB_HELIX_CLASS, PDB_HELIX_CLASS_LEN) == 0)
	  ...
	  if ( helix_length >= MIN_HELIX_LENGTH ) {
		...
		sequence[resctr].helix_type = helix_type;
	      }
	    }
	  }
...
static char single_letter (char code[]) {

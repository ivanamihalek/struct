...
#include "stride_sheet_mpat.h"

#define Minimum(x,y)  ((x)<(y) ? x : y)
#define Maximum(x,y)  ((x)<(y) ? y : x)

static int RightSide2(int L_A1, int L_D1, int LnkD, int LnkA,
		      int I1A, int I1D, int I2A, int I2D);
static int RightSide(int LnkA, int LnkD,
		     int I1A, int I1D, int I2A, int I2D );
static int RightSidePar(int LnkA, int LnkD,
			int I1A, int I1D, int I2A, int I2D);
static void JoinNeighbours(int *Lnk1A, int Res1,
			   int *Lnk1D, int Res2,
			   PATTERN **Nei, PATTERN *Pat,
			   int *MinDB1, int DB,
			   int *MinDW1, int DW,
			   int *Min, int j);
static void JoinNeighb(PATTERN **Nei, PATTERN *Pat,
		       int *MinDB2, int DB, int *MinDW2, int DW);
static int NearPar(int Res1, int Res2, int Res3, int Res4,
		   int Res5, int Res6, int Res7, int Res8,
		   char Cn1, char Cn2, char Cn3, char Cn4,
		   int *DistBest, int *DistWorst);
static int Near(int Res1, int Res2, int Res3, int Res4,
		int Res5, int Res6, int Res7, int Res8,
		char Cn1, char Cn2, char Cn3, char Cn4,
		int *DistBest, int *DistWorst);

/* in both stride_sheet_fasn.c and stride_sheet_mpat.c */
static void Alias(int *D1,int *A1,int *D2,int *A2,
		  char *D1Cn,char *A1Cn,char *D2Cn,char *A2Cn,
		  PATTERN *Pat) {

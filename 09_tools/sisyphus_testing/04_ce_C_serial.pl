...
$runce     = "$top_path/Downloads/ce_distr/CE";
$scratch   = "$top_path/scratch";
...
while (<IF>) {
    ...
    ###########################################################
    # tfm according to CE
    (-e "pom") || `ln -s /home/ivanam/Downloads/ce_distr/pom`;
    $ce_chainfile_renamed =  "pdbchains/$current_qry/$current_qry.to_$pdb_code$pdb_chain.ceC.0.pdb";
    $sys_ce_score = "none";
    #if ( ! -e $ce_chainfile_renamed ) {
	# run ce
	$cmd = "time $runce  -  $chainfile  -  $qryfile -  $scratch > tmp.out\n";
	if (system $cmd) {
	    print LOG "Error running $cmd.\n";
	}
	...
	$ce_chainfile_orig    = "$current_qry.to_$pdb_code$pdb_chain.ceC.0.pdb";
	...
	if (system $cmd) {
		...
		$ce_chainfile_renamed =  "pdbchains/$current_qry/$current_qry.to_$pdb_code$pdb_chain.ceC.$match_no.pdb";
		$cmd = "$pdtfm $qryfile tmp.$match_no.tfm > $ce_chainfile_renamed";
		if (system $cmd) {
		    print LOG "Error running $cmd.\n";
		}
	    }

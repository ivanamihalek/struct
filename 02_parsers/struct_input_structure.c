...
/*************************************************/
int db_input (FileWrapper * fptr, Descr * descr) {
    ...
    while(file_gets(line, BUFFLEN, fptr)!=NULL){
        ...
        if ( ! strncmp(line, "name", 4) ) {
            ...
            SSElement *element = descr->element + element_ctr;
    ...
    if ( !no_of_elements && file_feof(fptr) ) return -1;
...
/********************************************************/
int pdb_input (FileWrapper * fptr, char chain, Protein * protein, Descr * descr) {
...
int descr_shutdown (Descr * descr ) {
    ...
    if ( descr->element) {
        const int retval = free_element ( descr );
        if (retval) return retval;
    }
...
int check_input_type (FileWrapper *fptr) {
    ...
    if ( file_gets(line,LONGSTRING, fptr)) {
